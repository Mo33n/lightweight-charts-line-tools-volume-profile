import { Box, Point, pointInBox, HitTestResult, HitTestType, PaneCursorType, LineToolPaneView, AnchorPoint, interpolateLogicalIndexFromTime, logicalIndexToCoordinate, BaseLineTool, getToolCullingState, OffScreenState, deepCopy } from 'lightweight-charts-line-tools-core';
...
class ProfileHistogramRenderer {
    ...
    hitTest(x, y) {
        ...
        const box = new Box(new Point(Math.min(data.rangeXLeft, data.rangeXRight), Math.min(yHigh, yLow)), new Point((Math.max(data.rangeXLeft, data.rangeXRight) + histExtend), Math.max(yHigh, yLow)));
...
class RangeBoxRenderer {
    ...
    hitTest(x, y) {
        ...
        if (!data || !data.visible)
            return null;
        const box = new Box(new Point(Math.min(data.xLeft, data.xRight), Math.min(data.yTop, data.yBottom)), new Point(Math.max(data.xLeft, data.xRight), Math.max(data.yTop, data.yBottom)));
        if (pointInBox(new Point(x, y), box)) {
            return new HitTestResult(HitTestType.MovePointBackground, {
                pointIndex: null,
                suggestedCursor: PaneCursorType.Grabbing,
...
class FixedRangeProfilePaneView extends LineToolPaneView {
    ...
    _addAnchors(renderer) {
        ...
        if (geom) {
            ...
            anchorPoints.push(new AnchorPoint(geom.xLeft, (y0 ?? yMid ?? this._points[0].y), 0, false, PaneCursorType.EwResize), new AnchorPoint(geom.xRight, (y1 ?? yMid ?? this._points[1].y), 1, false, PaneCursorType.EwResize));
        }
        else {
...
function timeToScreenX(chart, series, timestamp) {
    const logical = interpolateLogicalIndexFromTime(chart, series, timestamp);
    if (logical == null)
        return null;
...
class LineToolFixedRangeProfile extends BaseLineTool {
...
export { DEFAULT_DELTA_PROFILE_TEMPLATE, DEFAULT_VOLUME_PROFILE_TEMPLATE, FIXED_RANGE_PROFILE_TOOL_TYPE, LINE_TOOL_FIXED_RANGE_PROFILE, LineToolFixedRangeProfile, METRIC_ROLE_OPTIONS, PROFILE_PROPERTY_SECTIONS, ProfilePropertiesEditor, attachProfilePropertiesEditor, barTimeToKey, bucketPrice, clampValueAreaFraction, clearProfilePipelines, cloneEditorState, computeRowStep, computeValueArea, createAverageBinnedAggregator, createDefaultFixedRangeProfileOptions, createMapFieldExtractor, createMaxBinnedAggregator, createOhlcVolumeExtractor, createSumBinnedAggregator, createTradesByPriceExtractor, defaultBinningAggregator, ensureBuiltinProfilePipelines, getProfileBarSource, getProfilePipeline, getPropertyPath, inferTickSizeFromBars, isOhlcBar, listProfilePipelines, mergeFixedRangeProfileOptions, mergeProfileTemplate, normalizeTimeRange, registerBuiltinProfilePipelines, registerProfileBarSource, registerProfilePipeline, resetBuiltinProfilePipelinesForTests, resolveProfile, resolveTickSize, sanitizeMetricValue, setPropertyPath, unregisterProfilePipeline };
//# sourceMappingURL=lightweight-charts-line-tools-volume-profile.js.map
